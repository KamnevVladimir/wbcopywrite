...
final class ClaudeService: @unchecked Sendable {
    ...
    private func getCategorySpecificGuidelines(_ category: Constants.ProductCategory) -> String {
        ...
        case .other:
            return """
            ОБЩИЕ РЕКОМЕНДАЦИИ:
            - Опирайся на предоставленное описание товара
            - Подчеркивай уникальные характеристики
            - Описывай выгоды и преимущества
            - Используй понятный и привлекательный язык
            - Добавляй эмоциональные триггеры по контексту
            """
        }
    }
